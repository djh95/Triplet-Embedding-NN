...
class TenNet_Tag_s1(nn.Module): # input batchSize * 1 * tagNum * tagNum
    def __init__(self, vocabulary_list, dropout_probability=0.5, filters=[3, 4, 5], filter_num=[100, 100, 100], in_channel=2, additional_matrix=None):
        ...
        for i in range(len(self.FILTERS)):
            self.branches.append(BasicConv2d(2, self.FILTER_NUM[i], kernel_size=(self.FILTERS[i], self.WORD_DIM)))
    ...
    def forward(self, tagsets):
        ...
        x = self.embedding_unstatic(torch.LongTensor(index_list)).view(-1,1,self.VOCAB_SIZE,self.WORD_DIM)

        if self.IN_CHANNEL == 2:
            x2 = self.embedding_static(torch.LongTensor(index_list)).view(-1,1,self.VOCAB_SIZE,self.WORD_DIM)
            x = torch.cat((x, x2), 1)
